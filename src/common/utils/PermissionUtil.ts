...
import { RESULTS, requestMultiple, Permission, check, PERMISSIONS } from "react-native-permissions";
...
class PermissionUtil {
    ...
    private cmmAccessDevicePlatformCheck = (): boolean => {
        return Platform.OS === "ios" || "android" ? true : false
    }
     ...
     */
    public cmmPermsArrCheck = async (permsCodeArr: Permission[]): Promise<Permission[]> => {
        ...
        for (const permsItem of permsCodeArr) {
            ...
            if (permsItem !== undefined || permsItem !== "unavailable") {

                //[STEP3] react-native-permissions 함수를 이용해 권한을 체크합니다.
                await check(permsItem)
                    .then((permsCheck) => {

                        console.log("permsCheck :: ", permsCheck);
        ...
        if (notGrantedArrLen == 0) return;
            ...
            await requestMultiple(notGrantedArr)
                .then((statues) => {
                    ...
                    notGrantedArr.forEach((permsItem) => statues[permsItem] === RESULTS.GRANTED ? permsCnt++ : 0);
